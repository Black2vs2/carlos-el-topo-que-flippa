...
func pingBackend() {
	ingestURL := client.ConfigGlobal.PublicIngestBaseUrls
	// Derive the backend base from the ingest URL (e.g. "http://localhost:3000/orders" -> "http://localhost:3000")
	parsed, err := url.Parse(strings.Split(ingestURL, ",")[0])
	if err != nil {
		log.Warnf("Could not parse ingest URL: %v", err)
		return
	}
	...
	if err != nil {
		log.Warnf("Backend ping failed: %v", err)
		log.Warn("Could not reach backend. The client will continue anyway.")
		return
	}
	...
	if err := json.Unmarshal(body, &result); err != nil || result.Status != "pong" || result.Client != "iStompedYou" {
		log.Warnf("Backend returned unexpected response: %s", string(body))
		return
	}

	log.Info("Backend connection OK — authenticated as stompedyou")
}
