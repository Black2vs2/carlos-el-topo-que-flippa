...
const Dialog: React.FunctionComponent<DialogProps> = ({ profitableOrders, confirmCallback }) => {
  ...
  const { isLoading, data, refetch, fetchStatus, isSuccess, isError } = useQuery({
    queryKey: ["clearDb"],
  ...
  useEffect(() => {
    if (isSuccess && data) {
      enqueueSnackbar(`Successfully cleared ${data.data?.count} ${singularOrPluralNaming(data.data?.count || 0, "order")}`, {
        autoHideDuration: 2000,
        variant: "success",
      });
      confirmCallback(selectedCityId);
      setOpen(false);
    }
  }, [isSuccess, data]);
  ...
  useEffect(() => {
    if (isError) {
      enqueueSnackbar("Error clearing city orders", { autoHideDuration: 2000, variant: "error" });
      setOpen(false);
    }
  }, [isError]);
